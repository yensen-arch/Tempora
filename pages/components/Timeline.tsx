...
const Timeline: React.FC<TimelineProps> = ({ videoRef, duration }) => {
  ...
  const handleDrag = (_, info: { point: { x: number } }) => {
    if (videoRef.current && containerRef.current) {
      ...
      if (!isNaN(newTime) && isFinite(newTime)) {
        videoRef.current.currentTime = Math.max(visibleStart, Math.min(visibleEnd, newTime))
      }
    }
  }
  ...
  const currentTrim = editHistory[editHistory.length - 1] || { start: 0, end: duration }
  const trimStartPercent = (currentTrim.start / duration) * 100
  const trimWidthPercent = ((currentTrim.end - currentTrim.start) / duration) * 100
    ...
    <div className="relative w-full max-w-3xl mt-4 bg-gray-100 rounded-lg p-8">
      ...
      <div className="relative">
        ...
        <div ref={containerRef} className="relative w-full h-20 overflow-hidden border-b border-gray-200">
          ...
          <div
            className="absolute h-12 bg-blue-200 bg-opacity-50"
            style={{
          ...
          <div className="absolute top-0 left-0 h-12 p-4 w-full">
            {Array.from({ length: Math.ceil((visibleEnd - visibleStart) / zoom) + 1 }).map((_, i) => {
              ...
              return (
                <div
                  ...
                  style={{ left: `${((time - visibleStart) / (visibleEnd - visibleStart)) * 100}%` }}
                >
